...
export default function GuestParty({ code }: { code: string }) {
  ...
  useEffect(() => {
    setGuestName(typeof window !== 'undefined' ? localStorage.getItem('karaoku_guest_name') || 'Guest' : 'Guest')
    ...
    const load = async () => {
      ...
      if (!p) {
        // Maybe user hasn't joined yet — redirect to /join/[code]
      ...
      const [{ data: q }, { count: gc }] = await Promise.all([
        supabase
          .from('queue_items')
          .select('*')
          .eq('party_id', p.id)
  ...
  return (
    <div style={{ minHeight: '100vh', background: s.black, paddingBottom: 80 }}>
      {/* HEADER */}
      <header
        style={{
          padding: '12px 16px',
